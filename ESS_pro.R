...
ESS.pro=function(y,x,nu,N,l,mcmc,brn,thin,tau.in,sig.in,xi0.in,xi1.in,xi.in,
  ...
  #  xi.in=pmax(0,mvrnorm(1,rep(0,N+1),K)) # need to choose carefully!
  xi.in = 0.05+rnorm(N+1,0,0.0005) 
...
#  if(verbose)
#    print("Initial MCMC sample draws:")
  ...
  for(i in 1:em){
    ...
    if(missing(xi.fix)){
      ...
      xi_out = ESS(xi_in,nu.ess,y_tilde,X,sig,cj,xi1)
    ...
    if(missing(xi1.fix)){ #draw from univariate truncated normal
      m1=sum(x*y1)/sum(x^2); s1=sqrt(sig/sum(x^2)) 
         #these values will change with new improved dependant prior on xi1
      u=runif(1,0,pnorm(-sum(cj*xi_out),m1,s1))
      xi1=m1+s1*qnorm(u)
    }
